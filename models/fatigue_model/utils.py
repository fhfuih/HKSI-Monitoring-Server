...
def get_generation_config():
    return dict(max_new_tokens=1024, do_sample=False, output_scores=True, return_dict_in_generate=True)
...
def process_sample(images, input_size=448, max_num=1):
    ...
    return pixel_values.to(torch.bfloat16).cuda(), num_patches_list
...
def process_scores_and_tokens(scores, token_ids, tokenizer, top_k=5):
    ...
    pairs = [(prob.item(), tokenizer.decode([idx.item()])) for prob, idx in zip(top_probs, top_indices)]
