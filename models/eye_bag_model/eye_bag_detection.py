...
class EyeBagDetection:
    ...
    def sort_points(self, points):
        ...
        return np.array(sorted_points, np.int32)
    ...
    def get_average_skin_color(self, image, landmarks):
        ...
        points = np.array(face_poly, dtype=np.int32)  # Use the first 68 points for the face
    ...
    def get_eye_bags_regions(self, landmarks, offset=5):
        ...
        left_eye_height = max(landmarks[41][1] - landmarks[37][1], landmarks[40][1] - landmarks[38][1])
        right_eye = [landmarks[42], landmarks[45], landmarks[46], landmarks[47]]
        right_eye_height = max(landmarks[47][1] - landmarks[43][1], landmarks[46][1] - landmarks[44][1])
        
        left_eye_bag = [(x, y + offset) for (x, y) in left_eye] + [(x, y + left_eye_height) for (x, y) in left_eye]
        #print(left_eye_bag)
        left_eye_bag = self.sort_points(left_eye_bag)
        right_eye_bag = [(x, y + offset) for (x, y) in right_eye] + [(x, y + right_eye_height) for (x, y) in right_eye]
    ...
    def detect_dark_circles(self, image, landmarks, average_skin_color, threshold=40):
        ...
        left_eye_color_distance = np.mean([self.color_distance(pixel, average_skin_color) for pixel in left_eye_pixels])
        right_eye_color_distance = np.mean([self.color_distance(pixel, average_skin_color) for pixel in right_eye_pixels])
        return left_eye_color_distance > threshold, right_eye_color_distance > threshold, left_eye_bag, right_eye_bag
    ...
    def run(self, image):
        ...
        has_dark_circles_left, has_dark_circles_right, left_eye_bag, right_eye_bag = self.detect_dark_circles(image, landmarks, average_skin_color)
                
        return has_dark_circles_left, has_dark_circles_right, left_eye_bag, right_eye_bag
