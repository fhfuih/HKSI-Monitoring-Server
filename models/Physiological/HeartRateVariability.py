...
class model_HRV:
    def __init__(
        ...
        fs: int,
        interpolate=False,
        time_data: Optional[Sequence[int]] = None,
        interp_freq: Optional[int] = None,
    ):
        """
        ...
        assert (not interpolate) or (interp_freq is not None and time_data is not None)

        self.meanRGB = np.array(meanRGB)  # Is this copy necessary?
        rPPG_cand = POS(self.meanRGB, fs)
        fs = int(fs)

        if interpolate:
            interp_freq = cast(int, interp_freq)
            time_data = cast(Sequence[int], time_data)
            interp_function = interp1d(
                time_data, rPPG_cand, "cubic"
            )  # discard 2 sec of initial signal
            x_new = np.arange(time_data[0], time_data[-1], 1 / interp_freq)
            rPPG_cand = interp_function(x_new)
            fs = interp_freq

        self.fs = fs
        self.rPPG = self.bandpass_filter(rPPG_cand, 5, 0.6, 4)
    ...
    def bandpass_filter(
        ...
        sos = butter(order, [min_freq, max_freq], btype="bandpass", output="sos", fs=fs)
    ...
    def inter_cleaning(self, signal, band):
        ...
        ppg1, mydict = self.bandpass_filter(
            signal, 5, 0.6, 3, apply_hanning=True, need_details=True
        )
    ...
    def HRV_measure(self, window_size=14.5, band=0.2, step=2.82):
        ...
        for i in range(L + 1):
            ...
            final_signal[running_window : win_end + 1] = (
                final_signal[running_window : win_end + 1] + cleaned_signal
            )
